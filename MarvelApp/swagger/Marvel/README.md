...
Each operation lives under the `MarvelClient` namespace and within an optional tag: `MarvelClient(.tagName).operationId`. If an operation doesn't have an operationId one will be generated from the path and method.
...
let getUserRequest = MarvelClient.User.GetUser.Request(id: 123)
...
Each [Request](#request) also has a `makeRequest` convenience function that uses `MarvelClient.shared`.
...
There are some options to control how invalid JSON is handled when decoding and these are available as static properties on `MarvelClient`:
...
- **MarvelClient.Characters**
	- **GetCharacterDetail**: GET `/v1/public/characters/{charid}`
	- **GetCharacterList**: GET `/v1/public/characters`
